...
class Mesh:
	def __init__(this, Format_, Ns,Nodes , Nt, Triangles, b_ext_size, Bord_exts, b_int_size, Bord_int, k, alpha):
		...
		this.Nodes_inter = []
		this.Nodes_inter = this.find_int_nodes()
		this.matrice_mass()
	...
	"""
	def u_inc(self, x, y):
		return np.exp(np.complex(0,1)*self.k*(x*np.cos(self.alpha + 0.0) + y*np.sin(self.alpha + 0.0)))
		...
		for p in range(0, this.Nt):
			...
			for i in range(0, 3):
				...
				for j in range(0, 3):
					J = this.Triangles[p].sommets[j]
					#print("--------------------------------------------------------------")
					#print("grad_phi_ref["+ str(i) + "]= " + str(this.grad_phi_ref[i] ))
					#print("grad_phi_ref["+ str(j) +"]= " + str(this.grad_phi_ref[j] ))
					#print("--------------------------------------------------------------")
					this.D[I-1][J-1] -= (this.aire_element(p+1) ) * np.dot( this.grad_phi_ref[j] ,np.dot(bTb, np.transpose(this.grad_phi_ref[i])))
	...
	def vector_U(this):
		...
		print(this.U)
		# for n in this.Nodes:
		# 	this.U[n.id -1 ] = this.U[n.id-1] + -this.u_inc(n.x, n.y)
...
def read_file(filename):
	...
	princeMesh = Mesh(MeshFormat, Number0fNodes, Nodes_, NumberOfTr , Trs_, cnt_ext, segs_ext , cnt_inter, segs_int,20, np.pi) #def __init__(this, Format_, Ns,Nodes , Nt, Triangles):
	return princeMesh

"""
Writing the solution and the mesh into a file readable by paraview
"""
def write_file( mesh_,filename):
...
#print(np.real(U))
np.savetxt("mat_D.csv",our_mesh.D,delimiter=",")
np.savetxt("mat_M.csv",our_mesh.M,delimiter=",")
np.savetxt("mat_A.csv",our_mesh.A,delimiter=",")
np.savetxt("mat_B.csv",our_mesh.b,delimiter=",")
np.savetxt("mat_U.csv",our_mesh.U,delimiter=",")
write_file(our_mesh,filename)
